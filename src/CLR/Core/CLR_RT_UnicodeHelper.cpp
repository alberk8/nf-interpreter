...
int CLR_RT_UnicodeHelper::CountNumberOfCharacters(int max)
{
    ...
    while (remaining > 0 && *pSrc)
    {
        ...
        if (ch == 0) // Treat embedded null as null terminator
        {
            break;
        }
        ...
        {
            // 3-byte sequence
            if (remaining >= 2 && pSrc[0] != 0 && pSrc[1] != 0)
            {
                int validCount = 0;
                if (UTF8_VALID_CONTINUATION(pSrc[0]))
                    validCount++;
        ...
        else if ((ch & 0xF8) == 0xF0)
        {
            ...
            if (remaining >= 3 && pSrc[0] != 0 && pSrc[1] != 0 && pSrc[2] != 0)
            {
                ...
                if (UTF8_VALID_CONTINUATION(pSrc[0]) && UTF8_VALID_CONTINUATION(pSrc[1]))
                    validCount++;
                if (UTF8_VALID_CONTINUATION(pSrc[0]) && UTF8_VALID_CONTINUATION(pSrc[1]) &&
                    UTF8_VALID_CONTINUATION(pSrc[2]))
                    validCount++;
    ...
    while (iMaxChars != 0 && iMaxBytes > 0)
    {
        ...
        else if ((ch & 0xF8) == 0xF0)
        {
            ...
            if (UTF8_VALID_CONTINUATION(ch2) && UTF8_VALID_CONTINUATION(ch3))
                validCount++;
            if (UTF8_VALID_CONTINUATION(ch2) && UTF8_VALID_CONTINUATION(ch3) && UTF8_VALID_CONTINUATION(ch4))
                validCount++;
